...
li2 = ["スライム", "ホイミスライム", "キングスライム", "はぐれメタル", "スライムベス"]
def create_mons():
    dc1 = {}
...
def mhpli():
    shp = dc2["スライム"]["HP"]; bhp = dc2["ホイミスライム"]["HP"]; khp = dc2["キングスライム"]["HP"]; hhp = dc2["はぐれメタル"]["HP"]; sbhp = dc2["スライムベス"]["HP"]
    return f"全体のHP...スライム{shp}, ホイミスライム{bhp}, キングスライム{khp}, はぐれメタル{hhp}, スライムベス{sbhp}"
...
ps = input("スタート")
mc = 0
alive_monsters = ["スライム", "ホイミスライム", "キングスライム", "はぐれメタル", "スライムベス"]
if not(ps == ("no")):
    winner = None
    while mc < 4:
        ...
        if pm == m1:
            ...
            print(f"{mons1}の残りHP{m1hp}　{mons2}の残りHP{m2hp}!  　　", end = ""); print(mhpli())
            if pm == "ホイミスライム":
                pa = input(f"入力(こうげき or ホイミ or ベホマ or まじんぎり) 残りMP{m1mp}")
            else:
                pa = input(f"入力(こうげき or ホイミ or まじんぎり) 残りMP{m1mp}")
            if pa == "こうげき":
                dod1 = dod(m1, m2)
                if dod1 == True:
                    print("回避成功")
